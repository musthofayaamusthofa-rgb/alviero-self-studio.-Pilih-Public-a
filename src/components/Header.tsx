  ...
  return (
    <header className={`hidden lg:block sticky top-0 z-40 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        >
          <div className="w-10 h-10 rounded-xl bg-slate-900 flex items-center justify-center text-white shadow-sm group-hover:scale-105 transition-all duration-300 border border-slate-800">
            <Camera className="w-5 h-5 text-emerald-400 group-hover:rotate-6 transition-transform" />
          </div>
          <div className="flex flex-col justify-center">
            <div className="flex items-center gap-1.5 flex-wrap leading-none">
              ...
              <span className="inline-flex items-center h-4.5 text-[8px] sm:text-[9px] bg-emerald-50 text-emerald-800 border border-emerald-200 font-extrabold px-2 rounded-full uppercase leading-none align-middle shadow-2xs">
        ...
        <nav className="hidden lg:flex items-center gap-1 bg-slate-100/90 p-1 rounded-full border border-slate-200/80 shadow-2xs">
          <button
            onClick={() => setActiveTab('katalog')}
        ...
        <div className="flex items-center gap-2.5">
          {onOpenBranchModal && (
            <button
              type="button"
              onClick={onOpenBranchModal}
              className="hidden sm:flex items-center gap-1.5 px-3.5 py-2 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-800 font-extrabold text-xs border border-slate-300/80 transition-all cursor-pointer active:scale-95"
            >
              <span>{currentBranchInfo.icon} {currentBranchInfo.shortName}</span>
              <span className="text-[10px] text-emerald-700 font-black">Ganti</span>
            </button>
          )}

          <button
            onClick={onOpenBooking}
            className="relative group bg-[#78b65d] hover:bg-[#5e9e44] text-white font-extrabold text-xs px-5 py-2.5 rounded-full shadow-sm hover:shadow-md transition-all duration-200 flex items-center gap-2 active:scale-95 cursor-pointer"
          >
